...
SSH wrapper with in-band PTY multiplexing. Two halves that talk to each other
over the byte stream: the Python wrapper (`src/shell_bucket/`) and the native V
`sb` binary (`native/sb/src/main.v`). Both are production code.
